...
// ============================================================
// CRITICAL SUICIDE & SELF-HARM PATTERNS
// These are ALL full explicit phrases — never single words.
// Single-word ambiguous terms (pills, bridge, knife, etc.)
// are intentionally NOT in this list to avoid false positives.
// ============================================================
const CRITICAL_SUICIDE_PATTERNS = [
  ...
  'i will take my own life',
  'tonight is my last night',
  'i already have the means to end',
  'i know how i will do it',
  'i want to end it all',
  'i want to disappear forever',
  'i dont want to be alive anymore',
  ...
  'i have a gun ready',
  'i have a knife ready to use on myself',
...
// ============================================================
// CRITICAL MESSAGE CHECK
// Returns true ONLY for explicit first-person crisis statements.
// Informational questions and third-party messages return false.
// ============================================================
const isCriticalMessage = (message: string): boolean => {
  // GUARD 1: Questions about suicide are never crisis messages
  if (isInformationalQuery(message)) return false;

  // GUARD 2: Messages about someone else are not the user's crisis
...
export const sendMessage = async (
  ...
  // ── GUARD 1: Informational queries go straight to AI — no crisis checks ──
  ...
  if (aboutSelf && isCriticalMessage(content)) {
    await adminDashboard.createAlert(
      ...
      {
        riskLevel: 'critical',
        riskFactors: ['Direct suicide/self-harm threat detected'],
  ...
  // ── GUARD 4: Enhanced risk analysis for non-critical messages ───────────
  let riskAnalysis;
  try {
    const conversationContext = {
      messages: convo.messages.map(msg => ({
